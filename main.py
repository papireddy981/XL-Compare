...
def JoinKeyRecommendation(cols1, cols2, common_columns):
    join_keys_prompt = f"""
You are provided with the columns of two versions of tables. 
Your task is to identify the **best join keys** for comparing the two tables. 

Rules:
- Prefer columns that uniquely identify each row (like Employee ID, Primary Key).
- If multiple columns are required together (composite key), suggest only that combination.
- Do NOT return all common columns — only the minimal unique key(s).
- If no clear key exists, return the most likely candidate.

table1 columns: {cols1}
table2 columns: {cols2}
common columns: {common_columns}

Return only the column name(s), comma separated. No explanation.
"""

    response = client.models.generate_content(
        model="gemini-2.0-flash",
        contents=join_keys_prompt,
    )
...
if file1 is not None and file2 is not None:
    ...
    if df1 is not None and df2 is not None:
        ...
        join_key_r = JoinKeyRecommendation(df1.columns.tolist(), df2.columns.tolist(), common_columns)

        print("join keys: ",join_key_r)
        st.write("Suggested join keys: ", join_key_r)
        
        if join_key_r is not None:
